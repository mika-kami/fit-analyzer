 ...
 *
 * FIT spec reference: ANT+ FIT Protocol, Profile.xlsx
 * Global message 26 = workout, global message 27 = workout_step
 *
 * To import: connect.garmin.com → Training → Workouts → Import
 */

// ── CRC-16 (ANT+ FIT polynomial 0xB2C0) ──────────────────────────────────
...
function strField(s, len) {
  ...
  return out; // null-terminated, zero-padded
}

// ── Definition message ────────────────────────────────────────────────────
// fields: [[fieldNum, fieldSize, baseType], ...]
// baseType: 0x02=uint8, 0x07=string, 0x84=uint16, 0x86=uint32
...
// ── FIT epoch ─────────────────────────────────────────────────────────────
const FIT_EPOCH = 631065600; // Unix ts of 1989-12-31 00:00:00 UTC
...
// ── Single workout step data message ─────────────────────────────────────
// intensity: 0=active 1=rest 2=warmup 3=cooldown 4=recovery 5=interval
// target_type: 1=heart_rate (with bpm range), 2=open (no target)
function wstep(idx, name, durationMs, bpmLo, bpmHi, intensity) {
  const targetType = (bpmLo > 0 && bpmHi > 0) ? 1 : 2;
  return [
    0x02,               // local msg 2 data header
    ...u16(idx),        // field 254: message_index
    ...strField(name, 16), // field 0: wkt_step_name
    ...u8(0),           // field 1: duration_type = 0 (time)
    ...u32(durationMs), // field 2: duration_value (milliseconds)
    ...u8(targetType),  // field 8: target_type
    ...u32(bpmLo),      // field 10: target_value_low (bpm)
    ...u32(bpmHi),      // field 11: target_value_high (bpm)
    ...u8(intensity),   // field 23: intensity
  ];
}

// ── Step sequences per workout type ──────────────────────────────────────
function buildSteps(day, z, sport) {
  const mps       = sportId(sport) === 1 ? 3.0 : 5.5; // running=3m/s cycling=5.5m/s
  const totalMs   = Math.max(30 * 60, (day.targetKm * 1000) / mps) * 1000;
  const mainMs    = (sub) => Math.max(10 * 60 * 1000, totalMs - sub);

  switch (day.type) {
    case 'recovery':
      return [wstep(0, 'Recovery', totalMs, z.z1.lo, z.z1.hi, 0)];

    case 'aerobic':
      return [
        wstep(0, 'Warmup',     15 * 60 * 1000, z.z2.lo, z.z2.hi, 2),
        wstep(1, 'Aerobic Z2', mainMs(30 * 60 * 1000), z.z2.lo, z.z2.hi, 0),
    ...
    case 'long':
      return [
        ...
        wstep(1, 'Long Z2',  mainMs(40 * 60 * 1000), z.z2.lo, z.z2.hi, 0),
    ...
    case 'interval': {
      const isVo2   = /vo.*2|vo₂/i.test(day.label ?? '');
      const workZ   = isVo2 ? z.z5 : z.z4;
      const workMin = isVo2 ? 4 : 8;
      const reps    = isVo2 ? 5 : 4;
      const steps   = [wstep(0, 'Warmup', 15 * 60 * 1000, z.z2.lo, z.z2.hi, 2)];
      for (let i = 0; i < reps; i++) {
        steps.push(wstep(steps.length, `Interval ${i+1}`, workMin*60*1000, workZ.lo, workZ.hi, 5));
        steps.push(wstep(steps.length, `Recovery ${i+1}`, 4*60*1000, z.z1.lo, z.z1.hi, 4));
      }
    ...
    case 'test':
      return [wstep(0, 'Assessment', totalMs, z.z2.lo, z.z2.hi, 0)];

    default:
      return [wstep(0, day.type ?? 'Workout', totalMs, z.z2.lo, z.z2.hi, 0)];
  }
}
...
export function buildFitWorkout(day, sport, maxHr = 180) {
  ...
  const fitTs  = Math.floor(Date.now() / 1000) - FIT_EPOCH;
  ...
  buf.push(0x00);  // local 0 data header
  buf.push(...u16(1), ...u16(0), ...u32(fitTs), ...u8(5));

  // ── Local 1: workout (global 26) ────────────────────────────────────────
  // field 0 = sport, field 2 = num_valid_steps, field 4 = wkt_name
  buf.push(...defMsg(1, 26, [
    [0,  1, 0x02],   // sport (uint8)
    [2,  2, 0x84],   // num_valid_steps (uint16)  ← field 2, NOT 6
    [4, 16, 0x07],   // wkt_name (string 16)
  ]));
  buf.push(0x01);  // local 1 data header
  buf.push(...u8(sId), ...u16(nSteps), ...strField(name, 16));
  ...
  buf.push(...defMsg(2, 27, [
    ...
    [2,    4, 0x86],  // duration_value (uint32): milliseconds
